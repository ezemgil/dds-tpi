...
async function getRandom(cantidad) {
    try {
        return await httpService.get(`${PELICULA_API_URL}/random?cantidad=${cantidad}`);
    } catch (error) {
...
async function removeCineasta(id_pelicula, id_cineasta) {
    try {
        ...
        console.error(
            `Error al eliminar el cineasta con id ${id_cineasta} de la película con id ${id_pelicula}`,
            error
        );
