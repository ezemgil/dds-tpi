...
const PeliculasNominaciones = ({ itemPelicula, Volver, Grabar }) => {
    ...
    useEffect(() => {
        premioService.getAll().then((response) => setListaPremios(response.data.premios));
    ...
    const onSubmit = () => {
        subaccionCRUD === "U" ? GrabarEdicionNominacion(nominacionItem) : GrabarNominacion(nominacionItem);
    };
    ...
    return (
        <div action="">
            ...
            <button
                className="badge d-flex p-2 m-2 align-items-center text-bg-dark border border-warning rounded-pill"
                onClick={() => AgregarNominacion()}
            >
                        ...
                        <tr key={nominacion.id}>
                            ...
                            <td className="d-flex gap-2 justify-content-center">
                                <button
                                    ...
                                    onClick={() => EditarNominacion(nominacion)}
                                >
                        ...
                        <div>
                            <div className="col form-group">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setNominacionItem({ ...nominacionItem, fecha_nominacion: e.target.value })
                                    }
                                />
                                {errors.fecha_nominacion && (
                                    <span className="text-danger">{errors.fecha_nominacion.message}</span>
                                )}
                            </div>
                            ...
                            <div className="col form-group">
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setNominacionItem({
                                            ...nominacionItem,
                                            premio: listaPremios.find((p) => p.id == e.target.value),
                                        })
                                    }
                                >
                                    <option disabled selected={nominacionItem.premio == null}>
                                        Seleccione una opcion
                                    </option>
                                    {listaPremios.map((premio) =>
                                        nominaciones.map((n) => n.premio.id).includes(premio.id) ? (
                                            premio.id === nominacionItem.premio?.id ? (
                                                <option
                                                    key={premio.id}
                                                    selected={premio.id == nominacionItem.premio?.id}
                                                    value={premio.id}
                                                >
                                                    {premio.nombre}
                                                </option>
                            ...
                            <div className="col form-group mt-4">
                                ...
                                <button
                                    className={`btn ${
                                        nominacionItem.fue_ganador ? "btn-success" : "btn-danger"
                                    } btn-sm rounded-pill ms-2`}
                                    onClick={() =>
                                        setNominacionItem({
                                            ...nominacionItem,
                                            fue_ganador: nominacionItem.fue_ganador ? 0 : 1,
                                        })
                                    }
                                    title={"fue_ganador"}
                                >
                                    <i
                                        className={`fa-solid ${
                                            nominacionItem.fue_ganador ? "fa-circle-check" : "fa-circle-xmark"
                                        }`}
                                    ></i>
                                </button>
                            </div>
                            ...
                            <div className="d-flex w-100 mt-4 justify-content-center">
                                <button
                                    ...
                                    className={`btn mx-2 text-dark ${
                                        nominacionItem.premio ? "btn-primary" : "btn-secondary"
                                    }`}
