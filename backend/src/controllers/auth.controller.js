...
export const login = async (req, res, next) => {
    ...
    if (!nombre || !clave) {
        log(req, "Error en login: Nombre de usuario o contraseña no proporcionados");
        return next(new BadRequestError("Nombre de usuario o contraseña no proporcionados"));
    }
    ...
    const claveCorrecta = usuario ? await bcrypt.compare(clave, usuario.clave) : false;

    // Verificar si el usuario existe y la contraseña es correcta
    if (!usuario || !claveCorrecta) {
        log(req, "Error en login: Nombre de usuario o contraseña incorrectos");
        return next(new BadRequestError("Nombre de usuario o contraseña incorrectos"));
    }

    // Generar el token de autenticación
    const accessToken = jwt.sign({ usuario: usuario.nombre, rol: usuario.rol.rol }, auth.accessTokenSecret, {
        expiresIn: auth.accessTokenExpiration,
    });

    // Generar el token de refresco
    const refreshToken = jwt.sign({ usuario: usuario.nombre, rol: usuario.rol.rol }, auth.refreshTokenSecret);
...
export const refresh = (req, res, next) => {
    ...
    jwt.verify(token, auth.refreshTokenSecret, (err, user) => {
        ...
        const accessToken = jwt.sign({ usuario: user.usuario, rol: user.rol }, auth.accessTokenSecret, {
            expiresIn: auth.accessTokenExpiration,
        });
...
export const logout = (req, res, next) => {
    ...
    if (!token && !refreshTokens.includes(token)) {
        ...
        return next(new BadRequestError("Token de refresco no proporcionado o inválido"));
    }
